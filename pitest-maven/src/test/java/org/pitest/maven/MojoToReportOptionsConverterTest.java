...
public class MojoToReportOptionsConverterTest extends BasePitMojoTest {
    ...
    public void testUsesSourceDirectoriesFromProject() {
        ...
        when(this.project.getTestCompileSourceRoots()).thenReturn(Arrays.asList("tst"));
        final ReportOptions actual = parseConfig("");
        assertEquals(Arrays.asList(new File("src"), new File("tst")), actual.getSourceDirs());
    }
    ...
    public void testParsesListOfMutationOperators() {
        ...
        "<mutators>" + //
        "                      <param>foo</param>" + //
        "                      <param>bar</param>" + "                  </mutators>";
        final ReportOptions actual = parseConfig(xml);
        assertEquals(Arrays.asList("foo", "bar"), actual.getMutators());
    }

    public void testParsersMutateStaticInitializersFlag() {
        assertTrue(parseConfig("<mutateStaticInitializers>true</mutateStaticInitializers>").isMutateStaticInitializers());
        assertFalse(parseConfig("<mutateStaticInitializers>false</mutateStaticInitializers>").isMutateStaticInitializers());
    }
    ...
    public void testParsesListOfClassesToAvoidCallTo() {
        ...
        "                      <param>foo</param>" + //
        "                      <param>bar</param>" + //
        "                      <param>foo.bar</param>" + "                  </avoidCallsTo>";
        final ReportOptions actual = parseConfig(xml);
        assertEquals(Arrays.asList("foo", "bar", "foo.bar"), actual.getLoggingClasses());
    }
    ...
    public void testParsesCommaListOfExcludedMethods() {
        ...
        "                      <param>foo*</param>" + //
        "                      <param>bar*</param>" + //
        "                      <param>car</param>" + "                  </excludedMethods>";
        final ReportOptions actual = parseConfig(xml);
    ...
    public void testObeysFailWhenNoMutationsFlagWhenPackagingTypeIsNotPOM() {
        ...
        assertTrue(parseConfig("<failWhenNoMutations>true</failWhenNoMutations>").shouldFailWhenNoMutations());
        assertFalse(parseConfig("<failWhenNoMutations>false</failWhenNoMutations>").shouldFailWhenNoMutations());
    }
    ...
    public void testParsesTestGroupsToExclude() {
        ...
        assertEquals(Arrays.asList("foo", "bar"), actual.getGroupConfig().getExcludedGroups());
    }
    ...
    public void testParsesSurefireConfigWhenFlagSet() {
        ...
        verify(this.surefireConverter).update(any(ReportOptions.class), any(Xpp3Dom.class));
    }

    public void testIgnoreSurefireConfigWhenFlagNotSet() {
        parseConfig("<parseSurefireConfig>false</parseSurefireConfig>");
        verify(this.surefireConverter, never()).update(any(ReportOptions.class), any(Xpp3Dom.class));
    }
    ...
    private ReportOptions parseConfig(final String xml) {
        try {
            ...
            @SuppressWarnings("unchecked") Predicate<Artifact> filter = Mockito.mock(Predicate.class);
            when(this.surefireConverter.update(any(ReportOptions.class), any(Xpp3Dom.class))).then(returnsFirstArg());
            this.testee = new MojoToReportOptionsConverter(mojo, this.surefireConverter, filter);
